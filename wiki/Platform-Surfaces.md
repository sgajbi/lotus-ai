...
13. `/platform/workflow-packs/queue-events` and `/platform/workflow-packs/queue-events/{queue_item_id}` expose durable queue admission, rejection, release, timeout, and cancellation evidence without exposing raw worker internals or replacing run-ledger, review-state, or task-flow posture,
14. the embedded `queue_attention` block reports source-backed workflow-pack queue heartbeat posture for active-admission saturation, stale active admissions, and durable timeout/cancellation queue events; retry-cluster attention remains a separate terminal-state slice,
