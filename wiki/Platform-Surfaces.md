...
13. `/platform/workflow-packs/queue-events` and `/platform/workflow-packs/queue-events/{queue_item_id}` expose durable queue admission, rejection, and release evidence without exposing raw worker internals or replacing run-ledger, review-state, or task-flow posture,
14. the embedded `queue_attention` block reports source-backed workflow-pack queue heartbeat posture for active-admission saturation and stale active admissions; timeout, cancellation, and retry-cluster attention remains a separate terminal-state slice,
