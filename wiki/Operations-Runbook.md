...
16. for retryable `POST /platform/workflow-packs/execute` commands, provide a stable business-operation `idempotency_key`. Same caller/tenant scope, key, and canonical input return the retained original response with the original AI request and workflow-pack run identity and `idempotency.status=REPLAYED`; changed input, an active duplicate, or an indeterminate prior outcome returns a coded `409` without another provider call. Omitted keys retain one-request/one-run compatibility. SQL-backed replay is part of workflow-pack run-store readiness and requires the `workflow_pack_execution_idempotency` table.
