...
def test_health_ready_degrades_for_unmigrated_sql_workflow_pack_task_flow_store(
    tmp_path: Path,
) -> None:
    database_url = f"sqlite:///{tmp_path / 'workflow-pack-task-flow-unmigrated.db'}"
    ...
    platform_body = platform_status.json()
    assert any(
        "workflow-pack task-flow store:" in warning
        for warning in platform_body["startup_readiness_warnings"]
    )
