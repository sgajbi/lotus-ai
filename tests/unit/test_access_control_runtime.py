...
def test_access_control_runtime_status_defaults_to_documentary_memory_mode() -> None:
    ...
    assert status.enforcement_state.value == "DOCUMENTARY_ONLY"
    assert status.store.status.value == "READY"
    assert status.policy_count >= 4
    assert status.tenant_isolation_active is False


def test_access_control_runtime_status_reports_sql_policy_resolution_ready(tmp_path: Path) -> None:
    ...
    assert status.enforcement_state.value == "POLICY_RESOLUTION_READY"
    assert status.store.status.value == "READY"
