...
def test_evaluation_runtime_status_reports_staged_assets() -> None:
    ...
    assert status.staged_fixture_count >= 7
    assert status.documented_fixture_count == 0
    assert status.staged_case_count == 15
    ...
    assert status.seam_coverage[1].staged_fixture_count == 2
    assert status.seam_coverage[1].staged_case_count == 5
    assert status.recorded_run_count == 3
    assert status.latest_recorded_run_id == "foundation_eval_2026_03_22_002"
