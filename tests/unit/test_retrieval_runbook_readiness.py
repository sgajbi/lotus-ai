from app.services.retrieval_runbook_readiness import build_retrieval_runbook_readiness


def test_retrieval_runbook_readiness_reports_operational_gaps() -> None:
    readiness = build_retrieval_runbook_readiness()

    assert readiness.service == "lotus-ai"
    assert readiness.runbook_ready is False
    assert readiness.required_item_count == 6
    assert readiness.completed_required_item_count == 3
    assert readiness.items[0].runbook_id == "retrieval_operational_runbook"
    assert readiness.items[0].status == "READY"
    assert readiness.items[1].status == "FOUNDATION_DOCUMENTED"
    assert readiness.items[2].status == "READY"
    assert readiness.items[3].status == "FOUNDATION_DOCUMENTED"
    assert readiness.items[4].status == "FOUNDATION_DOCUMENTED"
    assert readiness.items[5].status == "READY"
