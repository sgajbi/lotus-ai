...
def test_build_observability_runtime_status_returns_bounded_domain_summary() -> None:
    ...
    assert status.incident_evidence_supported_domain_count >= 1
...
def test_build_observability_runtime_status_flags_async_degradation() -> None:
    ...
    assert any("Dedicated incident-evidence summaries" in line for line in async_domain.status_summary)
