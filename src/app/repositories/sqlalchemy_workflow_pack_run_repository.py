...
class SqlAlchemyWorkflowPackRunRepository(SqlAlchemyRepositoryBase, WorkflowPackRunRepository):
    ...
    def save_run(self, record: WorkflowPackRunRecord) -> None:
        model = WorkflowPackRunModel(
            ...
            recovery_action_type=record.recovery_action_type,
            ...
            recovery_requested_by=record.recovery_requested_by,
            ...
            model_risk_status=record.model_risk_status,
            model_risk_approval_ref=record.model_risk_approval_ref,
            input_evidence_sha256=record.input_evidence_sha256,
            output_content_sha256=record.output_content_sha256,
            replay_nonce=record.replay_nonce,
    ...
    def _to_run_record(self, model: WorkflowPackRunModel) -> WorkflowPackRunRecord:
        return WorkflowPackRunRecord(
            ...
            recovery_action_type=model.recovery_action_type,
            source_queue_item_id=model.source_queue_item_id,
            recovery_decision_event_id=model.recovery_decision_event_id,
            recovery_attempt_number=model.recovery_attempt_number,
            source_workflow_pack_run_id=model.source_workflow_pack_run_id,
            recovery_requested_by=model.recovery_requested_by,
            recovery_evidence_ref=model.recovery_evidence_ref,
            evaluator_id=model.evaluator_id,
            evaluator_policy_version=model.evaluator_policy_version,
            provider_id=model.provider_id,
            model_id=model.model_id,
            model_version=model.model_version,
            model_risk_status=model.model_risk_status,
            model_risk_approval_ref=model.model_risk_approval_ref,
            input_evidence_sha256=model.input_evidence_sha256,
            output_content_sha256=model.output_content_sha256,
            replay_nonce=model.replay_nonce,
