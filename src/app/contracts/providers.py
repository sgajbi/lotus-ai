...
class CandidateUniverseExclusionReason(str, Enum):
    ...
    # A policy-ordered identity has no governed connection material
    # (issue #295): the policy says it may serve, but nothing says how to
    # reach it - excluded with the reason, never silently skipped.
    CONNECTION_MATERIAL_MISSING = "CONNECTION_MATERIAL_MISSING"
...
class CandidateUniverse(BaseModel):
    ...
    serving_policy_version: int | None = Field(
        default=None,
        description=(
            "Version of the governed serving-policy artifact this enumeration "
            "followed (issue #295, S2); null while ordering still comes from "
...
class RoutingDecisionDescriptor(BaseModel):
    ...
    serving_policy_version: int | None = Field(
        default=None,
        description=(
            "Version of the governed serving-policy artifact the enumeration "
            "followed (issue #295, S2); null while ordering comes from the "
            "configured pair."
        ),
    )
