...
7. RFC-0098 queue policy foundations now expose declarative per-pack queue policies, in-process queue admission capacity checks, bounded queue policy/status APIs, durable queue-event history for admission requests, queued posture, admitted posture, execution handoff, rejections, releases, timeout posture, cancellation posture, governed queue request-snapshot artifact refs, and governed retry/replay decision evidence behind memory or migration-backed SQL store modes, platform readiness reporting for that queue-event source, and runtime-status `queue_attention` for source-backed saturation, stale active-admission posture, durable terminal timeout/cancellation queue events, blocked retry/replay recovery posture, repeated timeout/cancellation/blocked-recovery clusters, and degraded queue-source posture when configured queue dependencies are not ready; actual retry/replay execution remains future work,
