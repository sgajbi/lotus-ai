...
7. RFC-0098 queue policy foundations now expose declarative per-pack queue policies, in-process queue admission capacity checks, bounded queue policy/status APIs, durable queue-event history for admission requests, grants, rejections, and releases behind memory or migration-backed SQL store modes, platform readiness reporting for that queue-event source, and runtime-status `queue_attention` for source-backed saturation and stale active-admission posture; terminal timeout, cancellation, and retry-cluster attention remains future work,
