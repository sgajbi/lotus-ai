...
7. RFC-0098 queue policy foundations now expose declarative per-pack queue policies, in-process queue admission capacity checks, bounded queue policy/status APIs, durable queue-event history for admission requests, grants, rejections, releases, timeout posture, and cancellation posture behind memory or migration-backed SQL store modes, platform readiness reporting for that queue-event source, and runtime-status `queue_attention` for source-backed saturation, stale active-admission posture, and durable terminal timeout/cancellation queue events; retry/replay execution and retry-cluster attention remain future work,
