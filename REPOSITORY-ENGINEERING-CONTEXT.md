...
7. RFC-0098 queue policy foundations now expose declarative per-pack queue policies, in-process queue admission capacity checks, bounded queue policy/status APIs, durable queue-event history for queue admission requests, queued posture, admitted posture, execution handoff, rejections, releases, timeout posture, cancellation posture, governed queue request-snapshot artifact refs, governed retry/replay decision evidence, and bounded retry/replay execution from retained request snapshots behind memory or migration-backed SQL store modes, platform readiness reporting for that queue-event source, and runtime-status `queue_attention` for source-backed saturation, stale active-admission posture, durable terminal timeout/cancellation queue events, blocked retry/replay recovery posture, repeated timeout/cancellation/blocked-recovery clusters, and degraded queue-source posture when configured queue dependencies are not ready; persisted queued-worker execution remains future work,
8. the service is designed to support Lotus apps without stealing domain ownership from them.
