...
7. RFC-0098 first-wave queue policy foundations now expose declarative per-pack queue policies, in-process queue admission capacity checks, bounded queue policy/status APIs, and runtime-status `queue_attention` for source-backed saturation and stale active-admission posture; durable queue-event history for timeout, cancellation, and retry-cluster attention remains future work,
