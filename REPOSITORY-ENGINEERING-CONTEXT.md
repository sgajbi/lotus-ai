...
6. RFC-0097 task-flow foundations now exist for future long-running workflow-pack paths: typed flow, step, checkpoint, blocking-condition, replacement-lineage, handoff, runtime-state, review-state, and evidence descriptors are available, bounded lifecycle transitions are centralized, task-flow plus checkpoint state can run in memory or through a migration-backed SQL store with platform readiness reporting, read-only task-flow catalog/detail/checkpoint routes now expose inspection, Phase-1 workflow-pack execution records task-flow/checkpoint state for explicit and implicit pack-backed execution paths, and workflow-pack review actions synchronize task-flow review posture plus replacement lineage; gateway adoption, Workbench adoption, heartbeat attention, and domain handoff execution remain future slices,
