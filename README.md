...
- caller identity and bounded capability policy now also resolve through a dedicated access-control registry seam, with runtime and governance posture exposed before broad enforcement is activated,
...
- caller access-control policy can now also move through a dedicated memory or SQL-backed registry seam, while broad request blocking remains deferred to later RFC-0012 slices.
