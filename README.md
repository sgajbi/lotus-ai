...
- caller identity and bounded capability policy now also resolve through a dedicated access-control registry seam, and task, retrieval, plus live-provider data-plane requests now enforce that registry explicitly,
...
- caller access-control policy can now also move through a dedicated memory or SQL-backed registry seam, with data-plane request blocking active and durable SQL-backed enforcement available when the access-control store uses SQLAlchemy.
