...
- retrieval indexing lifecycle is now more explicit too: retrieval job detail exposes deterministic chunking strategy, replay support, and persisted job events so blocked indexing states are inspectable,
...
- indexed retrieval ranking is now owned by the retrieval backend seam rather than assembled inside the gateway service layer, which keeps the path cleaner and prepares the SQL-backed implementation for eventual `pgvector` replacement,
...
- retrieval indexing can now also be exercised through the governed async-work model in stubbed mode, so the indexing path no longer depends only on direct synchronous execution,
