...
Every fence above is a single guarded `UPDATE` — a compare-and-set that holds on any SQL backend rather than an application lock — and is certified on the production engine by the PostgreSQL fence lane (`make test-postgres`, issue #344): the claim fence, the claim-instant rotation, release-versus-resume, reserve admission of the last hard-budget headroom, reconcile-releases-once, and settle-versus-hold each race two independent database sessions and admit exactly one winner. The lane runs in both merge gates against a `postgres:16-alpine` service and is fail-closed (an unreachable database fails it, never skips). A fence behaving differently on PostgreSQL than on SQLite is a P1 defect in the fence, not a test to adjust.
