...
6. circuit-breaker bookkeeping is keyed per **candidate** identity (`live_text_generation:<entry_id>`, issue #304): same-provider model candidates are normal serving topology, and a model-specific failure says nothing about the provider's other models — so one candidate's failures never open a sibling's breaker (the proof: model-A's repeated timeouts under the provider-keyed breaker excluded a healthy same-provider sibling within a single execution). Provider-wide outages open each candidate's breaker on its own failures — the conservative direction. Startup reconciliation carries pre-#304 provider-keyed OPEN circuits to every configured candidate of that provider (preserving the refusal is the safe direction; the pre-#176 bare-key row chains through the same migration); `RESET_DEGRADATION` and `RESET_ALL_PROVIDER_OPERATIONS` control actions clear every candidate's counters regardless of key shape
