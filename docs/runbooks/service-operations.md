...
- Workflow-pack synchronous execution: /platform/workflow-packs/execute
- Workflow-pack async execution submission: /platform/workflow-packs/execute-async
...
The FastAPI application enforces a service-owned HTTP boundary before request payloads reach task,
retrieval, workflow-pack, prompt, provider, or audit handlers.
...
1. host and CORS policy are owned by `lotus-ai` for direct service access, while ingress may add
   stricter environment controls,
2. HSTS is opt-in because TLS may terminate before the service container,
3. oversized requests return `413 application/problem+json` before endpoint handlers parse the
   body,
...
9. when `LOTUS_AI_WORKFLOW_PACK_QUEUE_EVENT_STORE_MODE=sqlalchemy`, confirm the embedded `workflow_pack_queue_event_store` block reports `READY` before treating queue admission, queued, admitted, execution-handoff, rejection, release, timeout, cancellation, request-snapshot artifact refs, retry-decision, and replay-decision history as restart-safe truth
...
13. verify `GET /platform/safety/evidence-readiness` when runtime safety approval posture matters
14. verify `GET /platform/safety/governance-status` when runtime safety rollout posture matters
...
9. when `LOTUS_AI_WORKFLOW_PACK_REGISTRY_STORE_MODE=sqlalchemy`, treat the embedded
...
10. when `LOTUS_AI_WORKFLOW_PACK_QUEUE_EVENT_STORE_MODE=sqlalchemy`, treat the embedded
   `workflow_pack_queue_event_store` block in `GET /platform/runtime-status` as the source gate for
   restart-safe queue-event history; if it is not `READY`, queue event catalog/detail routes should
   be treated as degraded and pack-backed execution should fail before queue admission side effects
11. keep workflow implementation changes in the owning repository; the `lotus-ai` control plane must
   not become a second editing surface for workflow behavior
12. inspect `GET /platform/workflow-packs/runs` to distinguish runtime completion posture from product review posture for recorded Phase-1 pack runs, use bounded repository-backed query filters such as `registration_ref`, `caller_app`, `tenant_id`, `workflow_surface`, `runtime_state`, `review_state`, `supportability_status`, `workflow_authority_owner`, and `limit` when operator triage should stay focused on one backlog slice, inspect `source_run_limit` and `source_run_count` in `filters_applied` to understand the bounded run window used for supportability-derived filtering, use the returned `ready_count`, `action_required_count`, and `historical_count` fields instead of recomputing supportability cohorts client-side, and rely on each run descriptor's `review_summary` block for latest review-transition actor, timestamp, and bounded history counts instead of reconstructing that provenance from raw event arrays
13. inspect `GET /platform/workflow-packs/runs/{run_id}` when support needs the exact registration ref, evidence descriptors, governed artifact refs, run-history events, bounded `allowed_review_actions`, shared review-progression posture, shared supportability posture, and bounded provenance summary attached to one pack execution; the run descriptor itself now also carries `supportability_status`, and the detail route now surfaces the same bounded review summary and bounded provenance summary used by downstream support contracts so callers do not need to parse review events or raw linkage sets just to understand review progression and provenance posture
14. inspect `GET /platform/workflow-packs/runs/{run_id}/operator-profile` when support needs one operator-facing summary of review pending, failure, expiry, supersession, partial-output, artifact, evidence, and bounded event-history posture for the run; the profile now also surfaces the latest recorded review-transition timestamp, actor, transition count, and a bounded provenance summary covering linked artifact and evidence types so support tooling does not need to parse raw event arrays or raw linkage sets just to understand review progression and provenance posture
15. inspect `GET /platform/workflow-packs/runs/{run_id}/consumer-view` when a downstream composition layer needs one grouped runtime-review-lineage-provenance-supportability contract candidate from lotus-ai without reconstructing those dimensions locally; the review block now also surfaces the latest recorded review-transition timestamp, actor, and bounded review-history counts so downstream product surfaces can render review provenance without parsing raw event history, and the `provenance_summary` block now exposes bounded linked artifact and evidence counts and types so downstream callers do not need to scan raw linkage arrays just to understand provenance posture
16. inspect `GET /platform/workflow-packs/source-events` or
   `GET /platform/workflow-packs/runs/{run_id}/source-events` when a portfolio-memory consumer needs
...
20. if the embedded `workflow_pack_run_store` block is not `READY`, treat `GET /platform/workflow-packs/runs`, `GET /platform/workflow-packs/runs/{run_id}`, `GET /platform/workflow-packs/source-events`, `GET /platform/workflow-packs/runs/{run_id}/source-events`, `GET /platform/workflow-packs/runs/{run_id}/consumer-view`, `GET /platform/workflow-packs/runs/{run_id}/operator-profile`, `POST /platform/workflow-packs/runs/{run_id}/review-actions`, `POST /platform/workflow-packs/execute`, and pack-backed `POST /ai/tasks/execute` requests as governed `503` degraded-state signals rather than as missing data, missing runs, or transient UI-only faults; the current bounded runtime now blocks those pack-backed execution paths before task execution and audit persistence so a failed preflight does not leave misleading post-failure run-side effects
21. inspect `GET /platform/workflow-packs/task-flows` with bounded repository-backed filters such as `workflow_pack_id`, `caller`, `tenant_id`, `workflow_surface`, `flow_status`, `supportability_status`, and `limit` when support needs task-flow posture for one backlog slice; run review synchronization also resolves source and replacement flows by bounded run-reference lookup instead of scanning the full task-flow catalog
22. when pack-backed `POST /platform/workflow-packs/execute` or `POST /ai/tasks/execute` returns a
...
23. for retryable `POST /platform/workflow-packs/execute` commands, provide a stable business-operation `idempotency_key`. Same caller and tenant scope plus the same key and canonical input returns the retained original response with `idempotency.status=REPLAYED`, including the original AI request and workflow-pack run identities, without a second provider call or duplicate run-side effects. The same scoped key with changed input returns `409`; an active, indeterminate, or integrity-invalid retained outcome also returns a coded `409` and does not start duplicate AI work. Omitted keys preserve one-request/one-run compatibility. SQL-backed replay uses the workflow-pack run store mode and requires the `workflow_pack_execution_idempotency` migration table. Each completed record carries a SHA-256 digest of its canonical retained response; a digest mismatch blocks replay instead of returning unverified AI evidence.
24. use `POST /platform/workflow-packs/execute-async` only when a workflow-pack execution should be persisted as a durable async runtime job; the route preserves eligibility, queue policy, request-snapshot evidence, and async job state before dedicated worker execution, accepts an optional request-level `idempotency_key`, replays the existing durable job for the same key and same execution fingerprint, and rejects the same key with different execution input as a `409` idempotency conflict
25. use `GET /platform/workflow-packs/queue-events/{queue_item_id}` when support needs the exact queue-item history from request through queued, admitted, running handoff, rejection, release, timeout, cancellation, degraded worker execution, retry decision, or replay decision; queue events may carry governed request-snapshot artifact refs for recovery investigation, but they do not embed raw task payloads; use the bounded retry/replay decision routes only to record explicit actor, reason, and evidence reference without executing workflow logic, use the bounded retry/replay execution routes only when the source queue item carries an executable request snapshot, and treat `/platform/workflow-packs/execute-async` as the source path for new durable queued-worker executions; retry/replay decision and execution requests must carry a `caller_app` authorized for async control-plane actions before recovery evidence is recorded; retry/replay execution requests may carry an `idempotency_key`, replay the first retained execution response for the same key and same operator-command fingerprint, and reject the same key with different retry/replay input as a `409` idempotency conflict; retry/replay-created runs carry bounded `recovery_lineage` back to the source queue item and decision event so support does not need to infer recovery posture from queue history alone; this source does not replace run-ledger, review-state, async job, or task-flow posture, and cancellation evidence does not claim the current synchronous execution body was interrupted
26. when runtime-status `queue_attention` surfaces terminal timeout, cancellation, degraded worker execution, blocked retry, blocked replay, repeated-failure cluster, or degraded queue-source posture, treat it as durable queue-boundary evidence for operator triage; event-derived counts are bounded by `event_sample_limit` and `event_sample_count`, and `event_window_truncated=true` means older durable queue events may exist outside the sampled window; retry/replay execution and queued-worker execution both reuse the normal workflow-pack execution path once retained request-snapshot evidence is available
27. when inspecting the embedded `workflow_pack_runtime.attention_queue` block in `GET /platform/runtime-status`, treat `queue_depth` as the full actionable backlog across executable pack versions and `items` as only the newest bounded sample up to `queue_limit`; if `queue_depth` exceeds `queue_limit`, continue triage through the ledger catalog rather than assuming the visible queue is the full backlog
...
Two evidence boundaries hold at the run seam (issue #332): claims write only when the CURRENT manifest version equals the version the run executed under (a queued run across a fixture change yields no evidence, with a logged refusal — proof-producing content is pinned, never re-established from newer declarations), and a claim is applicable only while its producing run exists as a COMMITTED `COMPLETED` PASS record — the write happens after the run's terminal persistence, and the read side independently verifies the committed run, so evidence orphaned by a mid-write failure is never honored. **The version guard's stated precondition**: `manifest_version` is an operator-maintained label, not a content digest - the manifest gate validates structure, not content-to-version binding, so an unbumped fixture edit (or a version flipped back) defeats the pin. The guard now ALSO compares a content-derived digest (issue #351): the loader computes `manifest_content_digest` over each family's id, declared proofs, and case-file bytes; runs capture it at submission and the writer refuses when EITHER the label or the digest differs — the digest catches unbumped content drift, the label catches undigested operator intent (a bumped label over identical content deliberately opens a new epoch). Historical runs without a digest stay guarded by the label alone. The manifest gate prints both. Bumping `manifest_version` on content changes remains the review convention.
...
**Execution ownership is single-owner** (issue #327): every approval atomically claims the action (`PENDING→CLAIMED`) before the effect runs and finalizes `CLAIMED→EXECUTED` with a durable result payload — a duplicated approval loses the claim and receives 409, never a second effect. A failed execution leaves the claim resumable ONLY by the claiming credential with the explicit `resume_interrupted_claim` field on the approval request; resuming is itself exclusive (the claim instant rotates under a compare-and-set, so concurrent resumes admit one winner before any effect, and a superseded still-running executor cannot finalize over the takeover). A claim whose credential disappears stays frozen by design — no timeout may re-open the one-owner window; recovery is the governed claim release (`/platform/governed-actions/claim-release-requests|claim-release-approvals`, issue #340): requester and approver must BOTH differ from the frozen holder (three distinct credentials), the pending hash pins the target's claim instant, and execution races the holder's own resume on the same compare-and-set fence — the resume wins any race, and the released action returns to PENDING with its requester evidence intact for a fresh distinct-credential approval. Erasure additionally proves signing readiness BEFORE any deletion (a missing receipt key means zero rows erased and 503, still approvable), and a lost erasure response replays the SAME signed receipt from the durable result without re-erasing.

The provider/governance transitions above are single guarded `UPDATE` operations — compare-and-set fences that hold on any SQL backend rather than an application lock. Seven are certified on PostgreSQL by the fence lane (`make test-postgres`, issue #344), each racing two independent database sessions and admitting exactly one winner: the claim fence, the claim-instant rotation, release-versus-resume, reserve admission of the last hard-budget headroom, reconcile-releases-once, settle-versus-hold for evidenced usage, and release-versus-hold for a proven non-billable attempt. The same lane separately certifies async immutable claim generations (issue #376), where a lease-row lock rechecks worker id, attempt id, and expiry before the transaction changes dependent rows: a crash-reclaimed job mints a new attempt, a reused stale worker is refused, and its terminal artifact metadata remains absent. The erasure behaviours described above — signing readiness before deletion, deterministic per-family recovery, and receipt replay from the durable result — are proven in the unit and integration lanes on SQLite and are **not** part of that PostgreSQL lane; do not cite the lane as evidence for them. Under REPEATABLE READ the provider/governance scenarios additionally force a real conflict and assert the observed SQLSTATE 40001 before the repository's retry converges — certification of the retry path, not an inference drawn from a one-winner outcome. The lane runs in both merge gates against a `postgres:16-alpine` service, is fail-closed (a missing or unreachable database fails it, never skips), and a fence failure produces a FAILED required check rather than a silently skipped dependent. A fence behaving differently on PostgreSQL than on SQLite is a P1 defect in the fence, not a test to adjust.
...
6. `async_runtime_content` and `artifact_content` carry **source-owned tenant attribution at creation** (issue #291): the submitting caller stamps async jobs, and worker-generated artifacts inherit the job's tenant. **The assertion trust model is caller-policy-bounded service assertion (issue #302)**: attribution is caller-asserted source truth — not cryptographically proven tenant identity — and the submission route only accepts an asserted `tenant_id` inside the authenticated caller's authorized tenant scope (`async_submission` capability: a restricted caller cannot assert a foreign tenant, and a caller whose policy requires tenant scope must assert one). Credential-bound tenant scope is the recorded upgrade path when platform credentials carry it. Erasure covers attributed rows (artifacts erase payload bytes AND metadata together); rows with `NULL` attribution — pre-attribution history and platform-owned content — are **counted on the receipt as `unattributable_count`, never erased, never inferred**, and leave only by the family's own time-bounded expiry
...
- A partially supplied alternate identity, `ordered_fallback` with no alternate, or an alternate that is the **same candidate** as the primary (same provider, model and revision — no failover at all) is a startup readiness finding (blocking in the promoted profile) and live executions are refused fail-closed with `INVALID_LIVE_CONFIGURATION`. A same-provider **sibling** model is a legitimate alternate (issue #314): the pair-era rejection of any same-provider alternate was an implementation limitation of the provider-keyed breaker, removed once breaker state became candidate-scoped (#304); provider diversity, where a deployment requires it, is an explicit resilience policy, not an identity-layer rule.

**Canonical serving-candidate identity (issue #314):** the human-readable entry id `provider:revision[:deployment]` is a row key, NOT the canonical identity — its components may contain the delimiter, and an actual collision exists (`text.local:qwen3:8b` renders identically for revision `qwen3:8b` without deployment and revision `qwen3` with deployment `8b`). The canonical identity is `candidate_id_v2` — a versioned opaque deterministic digest of the canonical serialization of the structured serving tuple (provider, model family, exact revision, deployment) — stamped on every catalogue entry from its structured fields, unique in persistence alongside a structural uniqueness invariant over the tuple itself (NULL deployment normalized to a sentinel), so neither an id-derivation regression nor delimiter games can collapse two logical candidates into one row. Never parse either identifier: the structured fields are authoritative. **The mutable serving fabric keys by the canonical identity**: new serving-policy rows store `cand2_...` ids (historical rows keep the v1 keys they were reviewed under, resolved by exact key — never rewritten), the candidate universe enumerates canonical ids, connection material resolves by them, and the P0 ambiguity block on policy additions is lifted — a delimiter-ambiguous v1 identity is safely admissible because the stored canonical id cannot collide. Operators keep naming candidates by the human row key on control surfaces; removal accepts either representation. Historical evidence keeps its historical identifiers — nothing is rewritten.

**The serving-policy artifact (issue #295, S2):** the ordered identities that may serve are a stored, versioned policy over catalogue entries — `GET /platform/providers/serving-policy` shows the operative version and its history; while none exists, ordering honestly follows the configured pair and the routing decision's `serving_policy_version` is null. **Adding** an identity is governed two-step (`POST /platform/providers/serving-policy/identity-additions` then `/identity-addition-approvals` under a DISTINCT verified credential; the hash binds the full resulting order, so any policy change before approval refuses the stale hash); **removing** one is the risk-reducing safety direction (`POST /platform/providers/serving-policy/identity-removals` — one verified principal, immediate, approver recorded as null). The policy applies under `ordered_fallback`; order is policy, never ranking. A policy identity without connection material is a reasoned `CONNECTION_MATERIAL_MISSING` exclusion, never a silent skip.

**Connection material beyond the pair (issue #295, S1):** `LOTUS_AI_PROVIDER_CONNECTIONS_JSON` declares per-identity connection entries (`{provider_id, model_id, model_version?, api_base, api_key_env?}`) keyed by the same catalogue identity the candidate universe enumerates. The primary/fallback settings pair seeds two entries (behaviour with only the pair configured is unchanged — pinned by test); a declared entry for a seeded identity deliberately overrides it; credentials are environment **references**, never secrets, and the resolved key reaches only the frozen execution config. A third identity resolves a full candidate config under the primary's shared protections (timeout, retries, output bound, enforcement) — this material answers "how do I connect to identity X", never "who serves first" (order is the S2 serving-policy artifact). **Deployment identity (issue #303):** a declared entry may carry `deployment` (participating in the catalogue entry-id derivation exactly as the catalogue derives it — `provider:revision:deployment`) and an informational `region`. A deployment-scoped identity is a DIFFERENT governed identity from the direct-API entry of the same provider and revision: it must be catalogued with its deployment to be eligible, and when it serves, binding, attempt debits, audit and the response all name the deployment-scoped entry id. `region` is operator-visible residency posture only — never an eligibility gate until a governed residency requirement dimension exists. `GET /platform/providers/routing-posture` lists every identity's resolved connection facts (`connection_identities`: provider, revision, deployment, region, endpoint host, credential reference NAME, seeded/declared) from the same merged map execution resolves through — the secret itself reaches no read surface. **The merged material map is the one connection authority (issue #298): the config a candidate actually executes under — on the ordered path and the fixed path alike — resolves through the same map configuration inspection reads, so a declared override of a seeded identity changes the actual endpoint and credential of the call, not just what inspection reports. An override replaces the connection whole: an entry without `api_key_env` executes keyless (the require-api-key fence then refuses) — the legacy secret never silently follows a replacement endpoint.** Malformed declarations are startup findings and refuse live execution fail-closed under both routing strategies.
...
6. circuit-breaker bookkeeping is keyed per **candidate** identity — since #314 the CANONICAL candidate id (`live_text_generation:<cand2_...>`), collision-proof by construction (issue #304 introduced the candidate scoping): same-provider model candidates are normal serving topology, and a model-specific failure says nothing about the provider's other models — so one candidate's failures never open a sibling's breaker (the proof: model-A's repeated timeouts under the provider-keyed breaker excluded a healthy same-provider sibling within a single execution). Provider-wide outages open each candidate's breaker on its own failures — the conservative direction. Startup reconciliation carries pre-#304 provider-keyed OPEN circuits to every configured candidate of that provider, and pre-#314 v1-candidate-keyed OPEN circuits to their canonical keys one-to-one (preserving the refusal is the safe direction; the pre-#176 bare-key row chains through the full migration); `RESET_DEGRADATION` and `RESET_ALL_PROVIDER_OPERATIONS` control actions clear every candidate's counters regardless of key shape
...
- Each attempt records a durable debit row (`provider_attempt_debits`, on the provider-operations store) under an idempotent identity keyed by the CANONICAL candidate — `adbt2:<execution_id>:<candidate_id_v2>:<attempt_index>` (issue #326; the legacy `adbt:<execution_id>:<candidate_entry_id>:<attempt_index>` form survives only for provider-keyed debits where no canonical candidate exists, and as readable history); the budget counter advances in the same transaction. An execution whose every attempt fails has still moved the envelope, and a crash after an attempt loses nothing. The identity segment is the **candidate** — the catalogue entry id binding provider, model revision and deployment (issue #299) — because two model candidates at the same provider are normal serving topology and a provider-keyed identity would collide their debits; the row also persists `candidate_entry_id`, `model_revision`, `attempt_index` and (since #314) the resolvable canonical `candidate_id_v2` so an audit names the full serving identity from the row alone (rows recorded before #299 carry null candidate columns and a provider-keyed debit id, honestly).
- **Hard-limit admission is an atomic per-attempt check-and-reserve** (issue #300): before the provider is called, the attempt reserves its governed maximum against the global budget row — input bounded by the request body's **byte count** (a provable ceiling: a byte-level token is at least one byte, unlike the request ceiling's ~4-bytes/token estimate) plus the enforced `max_output_tokens` — and the limit check plus counter advance are ONE guarded store statement, so concurrent replicas cannot jointly overshoot `LOTUS_AI_LIVE_TEXT_HARD_BUDGET_USD`. A refused admission is `BUDGET_EXCEEDED` with nothing written and no provider call. After the attempt, the reservation resolves in one transaction, in evidence order (issues #329/#346): provider-reported usage priced by an effective rate card settles to the evidenced `ACTUAL_USAGE` amount — the ONLY resolution that releases capacity at the boundary; a STATED non-billable attempt (429, pre-connect — the transport states `billable_risk` at the settle call, never inferred from pricing availability) releases to a zero-amount `RELEASED` row; everything else HOLDS as `UNRESOLVED_MAX` — the reserved maximum stays in the counter with the reservation's provenance intact. That covers unknown-usage timeouts/5xx, usage-withheld responses, AND a rate card that expired before settlement (a served-but-unpriceable attempt holds too; its observed usage is logged as `attempt_exposure_held_unpriceable` for reconciliation). A crash before resolution leaves the reservation counted — over-counting is the safe direction for a hard limit. The pre-execution budget preflight remains a fast-path refusal; the reservation is the enforcement point.
- Estimates are REPORTING posture, never settlement evidence: the response's failed-attempt figures still price the conservative estimate (~4 bytes/token input plus the `max_output_tokens` ceiling), but the durable row keeps the reserved maximum until real evidence arrives. **Unresolved exposure releases only through governed four-eyes reconciliation** — `POST /platform/provider-budget/reconciliation-requests` (names the debit, the provider-evidenced charge, and where to verify it; the pending hash pins the exposure's basis and held amount) then `-approvals` under a DISTINCT credential settles the row to `RECONCILED` and releases the difference exactly once; replays converge on the same evidenced outcome. An unpriceable candidate under an ENFORCED budget refuses admission outright (`require_priceable_admission`) — with enforcement off, no reservation and no debit is the explicit cost-unknown posture, never a silent zero.
...
- `max_estimated_cost_usd` is a **hard ceiling on the governed cost estimate** (issues #290, #301): one budget across every retry and fallback candidate, never reset. Before each attempt starts, the remaining ceiling must support that attempt at its conservative estimate (input estimate + `max_output_tokens`) under the candidate's own rate card — a cheaper fallback can fit where the primary would not, and consumption is exactly the durable attempt debits. A refused attempt is `REQUEST_COST_EXHAUSTED` (the caller's ceiling, not the platform envelope) and bills nothing; a declared ceiling on a candidate no rate card prices fails closed — an unverifiable guarantee never silently passes. **The claim matches the bound**: the input estimate is the documented ~4-bytes/token approximation (upgraded by same-execution provider-reported input) and is NOT a mathematical upper bound — a served attempt's actual provider-reported usage may exceed its pre-attempt estimate and settles as actual on the debit, so the ceiling bounds Lotus's governed pre-execution estimate, never actual provider billing. The provable bound (input ≤ request-body bytes) is enforced by the platform hard-budget reservation (issue #300), not by this per-request ceiling.
