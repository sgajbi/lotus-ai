...
11. inspect `GET /platform/workflow-packs/runs/{run_id}/operator-profile` when support needs one operator-facing summary of review pending, failure, expiry, supersession, partial-output, artifact, evidence, and bounded event-history posture for the run
12. inspect `GET /platform/workflow-packs/runs/{run_id}/consumer-view` when a downstream composition layer needs one grouped runtime-review-lineage-provenance-supportability contract candidate from lotus-ai without reconstructing those dimensions locally; the review block now also surfaces the latest recorded review-transition timestamp and actor so downstream product surfaces can render bounded review provenance without parsing raw event history
