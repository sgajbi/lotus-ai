...
10. inspect `GET /platform/workflow-packs/runs/{run_id}` when support needs the exact registration ref, evidence descriptors, governed artifact refs, run-history events, bounded `allowed_review_actions`, and shared supportability posture attached to one pack execution; the run descriptor itself now also carries `supportability_status` so catalog and detail consumers can render one consistent posture without re-deriving it client-side
