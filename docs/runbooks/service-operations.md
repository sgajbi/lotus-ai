...
**Connection material beyond the pair (issue #295, S1):** `LOTUS_AI_PROVIDER_CONNECTIONS_JSON` declares per-identity connection entries (`{provider_id, model_id, model_version?, api_base, api_key_env?}`) keyed by the same catalogue identity the candidate universe enumerates. The primary/fallback settings pair seeds two entries (behaviour at two candidates is unchanged — pinned by test); a declared entry for a seeded identity deliberately overrides it; credentials are environment **references**, never secrets, and the resolved key reaches only the frozen execution config. A third identity resolves a full candidate config under the primary's shared protections (timeout, retries, output bound, enforcement) — but **serving order stays the governed pair until the S2 serving-policy artifact lands**: this material answers "how do I connect to identity X", never "who serves first". Malformed declarations are startup findings and refuse ordered-fallback execution fail-closed.
