...
6. durable async execution with dedicated worker, queue, redrive, and quarantine support,
...
4. retrieval indexing, evaluation execution, document ingestion, and workflow-pack execution as active async consumers,
...
3. workflow-pack run ledger durability now exists for Phase-1 recorded runs, bounded review
   transitions are recorded durably, an explicit workflow-pack execution seam and reusable binding
   registry now exist for advisor brief, workspace rationale, TWR inspection support brief, the
   review-gated DPM PM memo contract over `lotus-manage` `DpmProofPackAiEvidenceInput`, the
   review-gated DPM wave PM memo contract over `lotus-manage` `DpmWaveReportInput`, and the
   review-gated outcome-review narrative contract over `lotus-manage` `DpmOutcomeAiEvidenceInput`;
   registration records remain the source of caller and workflow-surface scope while bindings stay
   limited to task-shape truth and are validated through the registry path. DPM PM memo execution
   adds deterministic payload guardrails for required proof-pack evidence fields, required forbidden
   actions, forbidden field rejection, forbidden requested-output rejection, unsupported
   requested-output rejection, and optional manage-owned `portfolio_memory_context` validation
   before run/audit/task-flow side effects. DPM wave PM memo execution adds deterministic payload
   guardrails for required wave report input fields, non-empty source refs, bounded wave items,
   proof-pack posture, no-external-execution posture, `NO_RAW_PAYLOADS`, forbidden action
   enforcement, forbidden field rejection, forbidden requested-output rejection, unsupported
   requested-output rejection, and optional manage-owned `portfolio_memory_context` validation
   before run/audit/task-flow side effects. Outcome-review narrative execution adds deterministic
   payload guardrails for required forbidden actions, forbidden field rejection, forbidden
   requested-output rejection, and optional manage-owned `portfolio_memory_context` validation
   before run/audit/task-flow side effects. The portfolio-memory path is bounded to source lineage:
   matching portfolio id, source content hash, capped event refs, `NO_RAW_PAYLOADS`, and explicit
   no-reconstruction source-authority policy. Run descriptors emit bounded `allowed_review_actions`
   plus bounded `review_summary` provenance, Phase-1 runs carry governed workflow-pack artifact refs
   ...
   direct latest-ready and latest-action-required run pointers plus bounded review provenance and
   bounded artifact or evidence linkage summaries, and a bounded cross-pack attention queue for the
   newest actionable runs now also carrying review provenance and bounded artifact or evidence
   linkage summaries, but broader downstream contract consumption is still incomplete,
4. RFC-0097 task-flow foundations now exist as contract and durable-state source truth for future long-running workflow-pack flows; lotus-ai now has a migration-backed task-flow store, checkpoint store, lifecycle transition guard, platform readiness reporting for the store mode, read-only task-flow catalog/detail/checkpoint routes, Phase-1 workflow-pack execution records task-flow/checkpoint state, run review actions synchronize task-flow review posture plus replacement lineage, accepted flows record `READY_FOR_HANDOFF` posture for the workflow authority owner, and `/platform/runtime-status` emits heartbeat-style task-flow attention for waiting, blocked, stale, and action-required flows, but no public mutation route or domain handoff execution runtime is implemented yet,
5. RFC-0098 queue-event foundations now exist as durable source truth for queue admission requests, queued posture, admitted posture, execution handoff, rejections, releases, timeout posture, cancellation posture, degraded queued-worker execution, governed queue request-snapshot artifact refs, governed retry/replay decisions, bounded idempotent retry/replay execution from retained request snapshots, and durable idempotent workflow-pack async execution submission through `/platform/workflow-packs/execute-async`; lotus-ai now has a migration-backed queue-event store, platform readiness reporting for the store mode, read-only queue-event catalog/detail routes, terminal timeout/cancellation/degraded-execution queue attention from durable events, blocked retry/replay recovery attention from durable decision events, repeated timeout/cancellation/blocked-recovery cluster attention, degraded queue-source posture in runtime-status when configured queue dependencies are not ready, same-key/same-payload replay and same-key/different-payload conflict semantics for async and retry/replay execution commands, and pack-backed execution preflights that reject when the configured queue-event source is not ready,
6. default-version resolution now exists as a conservative read-only control-plane route over
   registered, activation-eligible, non-superseded pack versions; broad production activation does
   not exist yet,
7. downstream owner repositories still have to carry the real workflow-bearing implementation and tests.
...
10. `RFC-0031` is implemented for the bounded Phase-1 workflow-pack runtime scope, with shared runtime direction, explicit execution, durable lineage-aware run posture, and governed live proof now in place across `lotus-ai`, `lotus-gateway`, `lotus-workbench`, `lotus-advise`, and `lotus-performance`; broader pack-family expansion remains follow-on work rather than an implementation blocker for this accepted slice.
11. `RFC-0032` is implemented for the bounded Phase-1 registry and activation scope, with registry, eligibility, onboarding, bounded operator-control slices, caller-policy-backed authorization evidence, and migration-backed durable registry state now in place for the current executable workflow-pack families; broader rollout posture and stronger enterprise authorization remain future work rather than blockers for this accepted slice.
12. `RFC-0033` is implemented for the bounded Phase-1 run-ledger and review-state scope, with durable storage, explicit Phase-1 execution, bounded review transitions, lineage-aware next-action contracts, grouped consumer and operator views, and merged downstream adoption slices now in place across advisor brief, workspace rationale, and TWR inspection support flows. The DPM proof-pack PM memo, DPM wave PM memo, and outcome-review narrative workflow packs are now executable and review-gated in `lotus-ai` for Manage-owned evidence, while Gateway and Workbench memo consumption remain follow-on work rather than blockers for the owner-side contract.
13. Platform `RFC-0097` Slices 1 through 7 are implemented locally as task-flow foundations: `src/app/contracts/workflow_pack_task_flows.py` defines flow, step, checkpoint, blocking-condition, replacement-lineage, handoff, and source-evidence descriptors, `src/app/services/workflow_pack_task_flow_contracts.py` defines bounded flow transitions, `src/app/services/workflow_pack_task_flow_service.py` plus the task-flow repository/store modules provide memory and SQL-backed task-flow plus checkpoint persistence with readiness-aware degradation, `/platform/workflow-packs/task-flows` plus detail/checkpoint routes expose read-only inspection, Phase-1 workflow-pack execution records task-flow/checkpoint state for explicit and implicit pack-backed execution paths, workflow-pack review actions synchronize task-flow review posture plus replacement lineage, accepted flows record `READY_FOR_HANDOFF` posture for the workflow authority owner, and `/platform/runtime-status` emits bounded heartbeat-style task-flow attention. Gateway adoption and Workbench adoption now exist on sibling RFC-0097 branches; domain handoff execution remains a future RFC-0097 slice.
