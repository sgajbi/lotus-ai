...
4. RFC-0097 task-flow contract foundations now exist as contract-only source truth for future long-running workflow-pack flows; no task-flow store, router, gateway endpoint, Workbench surface, heartbeat adapter, or domain handoff runtime is implemented yet,
...
13. Platform `RFC-0097` Slice 1 is implemented locally as a contract-only task-flow foundation: `src/app/contracts/workflow_pack_task_flows.py` defines flow, step, checkpoint, blocking-condition, replacement-lineage, handoff, and source-evidence descriptors, while `src/app/services/workflow_pack_task_flow_contracts.py` defines bounded flow transitions. Runtime storage, API routes, gateway adoption, Workbench adoption, heartbeat attention, and domain handoff execution remain future RFC-0097 slices.
