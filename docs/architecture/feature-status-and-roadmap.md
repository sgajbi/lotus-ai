...
2. workflow-pack registry activation state and control history can now run durably through the SQL-backed registry store, the registry inspection surfaces now also expose bounded explicit execution-binding metadata for implemented pack versions, but broader workflow-pack runtime adoption is still incomplete,
