...
4. RFC-0097 task-flow foundations now exist as contract and durable-state source truth for future long-running workflow-pack flows; lotus-ai now has a migration-backed task-flow store, checkpoint store, lifecycle transition guard, platform readiness reporting for the store mode, and read-only task-flow catalog/detail/checkpoint routes, but no public mutation route, gateway endpoint, Workbench surface, heartbeat adapter, or domain handoff runtime is implemented yet,
...
13. Platform `RFC-0097` Slices 1 through 3 are implemented locally as task-flow foundations: `src/app/contracts/workflow_pack_task_flows.py` defines flow, step, checkpoint, blocking-condition, replacement-lineage, handoff, and source-evidence descriptors, `src/app/services/workflow_pack_task_flow_contracts.py` defines bounded flow transitions, `src/app/services/workflow_pack_task_flow_service.py` plus the task-flow repository/store modules provide memory and SQL-backed task-flow plus checkpoint persistence with readiness-aware degradation, and `/platform/workflow-packs/task-flows` plus detail/checkpoint routes expose read-only inspection. Public mutation routes, gateway adoption, Workbench adoption, heartbeat attention, and domain handoff execution remain future RFC-0097 slices.
