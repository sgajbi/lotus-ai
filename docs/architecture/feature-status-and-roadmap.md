...
5. RFC-0098 queue-event foundations now exist as durable source truth for queue admission requests, grants, rejections, releases, timeout posture, and cancellation posture; lotus-ai now has a migration-backed queue-event store, platform readiness reporting for the store mode, read-only queue-event catalog/detail routes, terminal timeout/cancellation queue attention from durable events, and pack-backed execution preflights that reject when the configured queue-event source is not ready, but retry/replay execution and retry-cluster attention remain a future slice,
