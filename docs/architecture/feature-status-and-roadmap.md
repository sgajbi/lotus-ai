...
4. RFC-0097 task-flow foundations now exist as contract and durable-state source truth for future long-running workflow-pack flows; lotus-ai now has a migration-backed task-flow store, checkpoint store, lifecycle transition guard, platform readiness reporting for the store mode, read-only task-flow catalog/detail/checkpoint routes, Phase-1 workflow-pack execution records task-flow/checkpoint state, and run review actions synchronize task-flow review posture plus replacement lineage, but no public mutation route, gateway endpoint, Workbench surface, heartbeat adapter, or domain handoff runtime is implemented yet,
...
13. Platform `RFC-0097` Slices 1 through 5 are implemented locally as task-flow foundations: `src/app/contracts/workflow_pack_task_flows.py` defines flow, step, checkpoint, blocking-condition, replacement-lineage, handoff, and source-evidence descriptors, `src/app/services/workflow_pack_task_flow_contracts.py` defines bounded flow transitions, `src/app/services/workflow_pack_task_flow_service.py` plus the task-flow repository/store modules provide memory and SQL-backed task-flow plus checkpoint persistence with readiness-aware degradation, `/platform/workflow-packs/task-flows` plus detail/checkpoint routes expose read-only inspection, Phase-1 workflow-pack execution records task-flow/checkpoint state for explicit and implicit pack-backed execution paths, and workflow-pack review actions synchronize task-flow review posture plus replacement lineage. Gateway adoption, Workbench adoption, heartbeat attention, and domain handoff execution remain future RFC-0097 slices.
