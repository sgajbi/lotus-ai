...
5. RFC-0098 queue-event foundations now exist as durable source truth for queue admission requests, queued posture, admitted posture, execution handoff, rejections, releases, timeout posture, cancellation posture, governed queue request-snapshot artifact refs, governed retry/replay decisions, and bounded retry/replay execution from retained request snapshots; lotus-ai now has a migration-backed queue-event store, platform readiness reporting for the store mode, read-only queue-event catalog/detail routes, terminal timeout/cancellation queue attention from durable events, blocked retry/replay recovery attention from durable decision events, repeated timeout/cancellation/blocked-recovery cluster attention, degraded queue-source posture in runtime-status when configured queue dependencies are not ready, and pack-backed execution preflights that reject when the configured queue-event source is not ready, but persisted queued-worker execution remains a future slice,
