...
5. RFC-0098 queue-event foundations now exist as durable source truth for queue admission requests, grants, rejections, and releases; lotus-ai now has a migration-backed queue-event store, platform readiness reporting for the store mode, read-only queue-event catalog/detail routes, and pack-backed execution preflights that reject when the configured queue-event source is not ready, but terminal timeout, cancellation, and retry-cluster attention remains a future slice,
