...
13. Platform `RFC-0097` Slices 1 through 6 are implemented locally as task-flow foundations: `src/app/contracts/workflow_pack_task_flows.py` defines flow, step, checkpoint, blocking-condition, replacement-lineage, handoff, and source-evidence descriptors, `src/app/services/workflow_pack_task_flow_contracts.py` defines bounded flow transitions, `src/app/services/workflow_pack_task_flow_service.py` plus the task-flow repository/store modules provide memory and SQL-backed task-flow plus checkpoint persistence with readiness-aware degradation, `/platform/workflow-packs/task-flows` plus detail/checkpoint routes expose read-only inspection, Phase-1 workflow-pack execution records task-flow/checkpoint state for explicit and implicit pack-backed execution paths, workflow-pack review actions synchronize task-flow review posture plus replacement lineage, and `/platform/runtime-status` emits bounded heartbeat-style task-flow attention. Gateway adoption and Workbench adoption now exist on sibling RFC-0097 branches; domain handoff execution remains a future RFC-0097 slice.
