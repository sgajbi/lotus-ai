...
2. required evidence items remain not ready in foundation phase,
3. prompt governance review now includes evidence posture as a first-class blocking area alongside technical and operational readiness.
...
4. header-only identity may exercise all-tenant access only when the default-closed
   `local_header_caller_identity_enabled` security setting is explicitly enabled for a local
   runtime; readiness settings do not alter authorization,
